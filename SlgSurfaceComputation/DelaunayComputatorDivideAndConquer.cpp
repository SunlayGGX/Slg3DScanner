...
{
    DelaunayTriangle::IndexType diff = endIndex - beginIndex + 1;
    if(diff > 3)
    {
        std::list<DelaunayTriangle> mergeContainer1;
        std::list<DelaunayTriangle> mergeContainer2;

        DelaunayTriangle::IndexType half = beginIndex + (diff - 1) / 2;
...
DelaunayTriangle::IndexType DelaunayComputator::findBottomPointIndex(const std::list<DelaunayTriangle>& inContainer, bool isLeftContainer) const
{
    float bottomValue = std::numeric_limits<float>::max();
    DelaunayTriangle::IndexType result = 0;
    ...
    if(isLeftContainer)
    {
        ...
        for(auto iter = inContainer.begin(); iter != endIter; ++iter)
        {
            ...
            if(iter->index1 != std::numeric_limits<DelaunayTriangle::IndexType>::max())
            {
            ...
            if(iter->index2 != std::numeric_limits<DelaunayTriangle::IndexType>::max())
            {
...
bool DelaunayComputator::isInsideCircumcenterCircle(DelaunayTriangle::IndexType pointIndex, const DelaunayTriangle& triangleCheck) const
{
...
DelaunayTriangle::IndexType DelaunayComputator::findCandidatePoint(const DelaunayEdge& baseEdge, std::list<DelaunayTriangle>& inOutContainer, const bool isLeftContainer) const
{
    std::list<DelaunayTriangle::IndexType> indexCandidates;

    DelaunayTriangle::IndexType baseIndex = isLeftContainer ? baseEdge.index0 : baseEdge.index1;
    ...
    //retrieve candidate triangles
    {
        ...
        {
            DelaunayTriangle::IndexType base;
            DelaunayTriangle::IndexType indexA;
            DelaunayTriangle::IndexType indexB;
            ...
            if(indexA != std::numeric_limits<DelaunayTriangle::IndexType>::max() && m_pointList[indexA].getProjectedPosition().y > m_pointList[base].getProjectedPosition().y && std::find(indexCandidates.begin(), indexCandidates.end(), indexA) == indexCandidates.end())
            {
                indexCandidates.emplace_back(indexA);
            }

            if(indexB != std::numeric_limits<DelaunayTriangle::IndexType>::max() && m_pointList[indexB].getProjectedPosition().y > m_pointList[base].getProjectedPosition().y && std::find(indexCandidates.begin(), indexCandidates.end(), indexB) == indexCandidates.end())
            {
        ...
        {
            return std::numeric_limits<DelaunayTriangle::IndexType>::max();
        }
    }
    ...
    indexCandidates.sort(
        [&pointList = this->m_pointList, &baseIndex, &vectBaseEdge, &basePoint](DelaunayTriangle::IndexType id1, DelaunayTriangle::IndexType id2)
    {
...
void DelaunayComputator::mergeSubsetsPointsIntoOne(std::list<DelaunayTriangle>& inContainer1, std::list<DelaunayTriangle>& inContainer2, std::list<DelaunayTriangle>& outMergeContainer)
{
    ...
    for(;;)
    {
        ...
        if(!(candidateEdgeQuad.index0 == std::numeric_limits<DelaunayTriangle::IndexType>::max() || candidateEdgeQuad.index1 == std::numeric_limits<DelaunayTriangle::IndexType>::max()))
        {
            /*
        ...
        else
        {
            ...
            if(candidateEdgeQuad.index0 == std::numeric_limits<DelaunayTriangle::IndexType>::max())
            {
                zip.emplace_back(baseEdge.index0, baseEdge.index1, candidateEdgeQuad.index1);
                baseEdge.index1 = candidateEdgeQuad.index1;
            }
            else if(candidateEdgeQuad.index1 == std::numeric_limits<DelaunayTriangle::IndexType>::max())
            {
    ...
    {
        if(iter->index0 == std::numeric_limits<DelaunayTriangle::IndexType>::max() || iter->index1 == std::numeric_limits<DelaunayTriangle::IndexType>::max() || iter->index2 == std::numeric_limits<DelaunayTriangle::IndexType>::max())
        {
            auto toErase = iter++;
